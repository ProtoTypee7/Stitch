...
namespace P2WebMVC.Controllers
{
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            try
            {
                await mailService.SendEmailAsync(model.Email, "Forgot Password", "Your OTP is 1234", true);
                return RedirectToAction("Login");
            }
            catch (Exception ex)
            {
                ViewBag.ErrorMessage = $"Send failed: {ex.Message}";
        ...
        public async Task<IActionResult> Wishlist()
        {
            ...
            var wishlist = await sqlDbContext.Wishlists
        ...
        public async Task<IActionResult> AddToWishlist(Guid productId)
        {
            ...
            var userExists = await sqlDbContext.Users.AnyAsync(u => u.UserId == userId);
            if (!userExists)
                return NotFound("User not found");

            var productExists = await sqlDbContext.Products.AnyAsync(p => p.ProductId == productId);
            if (!productExists)
            ...
            if (wishlist == null)
            {
                ...
                sqlDbContext.Wishlists.Add(wishlist);
                await sqlDbContext.SaveChangesAsync(); // Needed to get WishlistId
            }
        ...
        public async Task<IActionResult> RemoveFromWishlist(Guid userId, Guid productId)
        {
            ...
            var item = wishlist?.WishlistProducts.FirstOrDefault(wp => wp.ProductId == productId);
            if (item != null)
            {
            ...
            return RedirectToAction("Wishlist", new { userId });
        }
        ...
        public async Task<IActionResult> AccountSettings()
        {
            ...
            var user = await sqlDbContext.Users.FindAsync(userId);
            if (user == null)
                return NotFound();
        ...
        public IActionResult SignOut()
        {
            ...
            HttpContext.Response.Cookies.Delete("Authorization_Token_Trinkle");
